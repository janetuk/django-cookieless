...
class CookielessSessionMiddleware:
    ...
    def nocookies_response(self, request, response):
        ...
        if request.session.session_key and not request.path.startswith(
            ...
            # Force HTTP response content into Unicode..
            if hasattr(response, "content"):
                http_content = UnicodeDammit(response.content)
                markup = http_content.unicode_markup or ""
                # -> utf-8
                if (repl_form not in markup):
                        repl_form = """%s value="%s" />
                                       </form>""" % (
                            repl_form,
                            session_key,
                ...
                try:
                    response.content = self._re_forms.sub(
                        repl_form, response.content.decode('ascii')
                    )
                except:
                    pass
            return response
